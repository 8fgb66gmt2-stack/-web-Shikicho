document.addEventListener('DOMContentLoaded',()=>{
 ...
 if(form)form.addEventListener('submit',e=>{e.preventDefault();const value=input.value.trim();if(value==='雛菊'||value==='四季を絶やすこと'||value==='四時を絶やすこと勿れ'){content.classList.add('unlocked');error.textContent='認証成功。機密資料への閲覧権限を確認しました。'}else{content.classList.remove('unlocked');error.textContent='認証失敗。認証キーワードを確認してください。'}});
